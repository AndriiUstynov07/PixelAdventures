...
public class IceBoss extends Boss {
    // Size is scaled relative to MiniBossIceKnight so that the ratio matches
    ...
    public IceBoss(float x, float y) {
    ...
    private boolean isPlayerInRoom6() {
        if (target == null) return false;

        // Borders of room 6 (expanded like in the first level boss logic)
        float roomLeft = 530.76f;
        float roomRight = 731.62f;
        float roomBottom = 575.40f;
        float roomTop = 732.80f;
    ...
    private void constrainToRoom6() {
        // Expanded borders for room 6 where the boss is located
        float roomLeft = 480.76f;
        float roomRight = 731.62f;
        float roomBottom = 495.40f;
        float roomTop = 732.80f;
    ...
    private void renderHealthBar(SpriteBatch batch) {
        ...
        float barWidth = 70f*0.78f; // Ширина смужки
        float barHeight = 8f*0.78f; // Висота смужки
    ...
    public Rectangle getBounds() {
        ...
        float collisionMargin = 10f; // Зменшуємо межі на 30 пікселів з кожного боку
