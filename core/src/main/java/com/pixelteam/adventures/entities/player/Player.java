...
public class Player extends Character {
    ...
    private void initializePlayableAreas() {
        ...
        if (healthBarScaleFactor == 1.0f) { // Level 1
            ...
            // Розширена основна ігрова зона для рівня 2
            // Включає точку спавну (995, 65) і більше простору для руху
            playableAreas.add(new Rectangle(
                750,    // x - лівий край зони (зменшено для більшої області)
                30,     // y - нижній край зони (зменшено для більшої області)
                450,    // width - ширина зони (збільшено)
                350     // height - висота зони (збільшено)
            ));

            // Розширена додаткова зона зліва
            playableAreas.add(new Rectangle(
                550,    // x (зменшено для більшої області)
                30,     // y (зменшено для більшої області)
                200,    // width
                350     // height (збільшено)
            ));

            // Розширена додаткова зона справа
            playableAreas.add(new Rectangle(
                1200,   // x
                30,     // y (зменшено для більшої області)
                100,    // width (збільшено)
            ...
            // Розширена верхня зона
            playableAreas.add(new Rectangle(
                750,    // x (зменшено для більшої області)
                380,    // y (збільшено для кращого з'єднання з коридором)
                450,    // width (збільшено)
                250     // height (збільшено)
            ));

            // Розширений коридор між основною і верхньою зонами
            playableAreas.add(new Rectangle(
                900,    // x (зменшено для центрування)
                330,    // y (збільшено для кращого з'єднання з зонами)
                150,    // width (збільшено)
                50      // height
            ));
        }
